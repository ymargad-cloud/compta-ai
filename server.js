...
const handler = async (req, res) => {
  ...
  // GET /api/companies — pagination automatique pour lever la limite de 20
  if (req.method === 'GET' && url === '/api/companies') {
    const user = await getUser(req);
    if (!user) return send(res, 401, { error: 'Non authentifié' });
    let allCompanies = [];
    if (user.role === 'admin') {
      // Paginer par tranches de 20 jusqu'à tout récupérer
      let offset = 0;
      while(true) {
        const r = await supa('GET', 'companies', { filter: `order=name.asc&limit=20&offset=${offset}` });
        const batch = Array.isArray(r.data) ? r.data : [];
        allCompanies = allCompanies.concat(batch);
        if (batch.length < 20) break;
        offset += 20;
        if (offset > 2000) break; // sécurité
      }
    } else {
      const links = await supa('GET', 'user_companies', { filter: `user_id=eq.${user.id}&limit=500` });
      const ids = (Array.isArray(links.data) ? links.data : []).map(l => l.company_id);
      if (!ids.length) return send(res, 200, []);
      // Paginer par tranches pour les ids aussi
      let offset = 0;
      while(true) {
        const r = await supa('GET', 'companies', { filter: `id=in.(${ids.join(',')})&order=name.asc&limit=20&offset=${offset}` });
        const batch = Array.isArray(r.data) ? r.data : [];
        allCompanies = allCompanies.concat(batch);
        if (batch.length < 20) break;
        offset += 20;
  ...
  // Debug companies — voir exactement ce que Supabase retourne
